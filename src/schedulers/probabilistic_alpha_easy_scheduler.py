...
def convex_merge(d1, d2, r):
    # a convex combination of two ditributions
    assert 0 <= r <= 1
    result = Distribution()
    max_key = max(max(d1.bins.keys()), max(d2.bins.keys()))
    result.emptytouch(max_key)
    d1.emptytouch(max_key)
    d2.emptytouch(max_key)

    result.number_of_jobs_added = r * d1.number_of_jobs_added + (1-r)* d2.number_of_jobs_added
    while max_key > 1:
...
class Distribution(object):
    ...
    def emptytouch(self, time): # just add empty bins
        rounded_up_time = pow(2, int(log(max(2 * time - 1, 1), 2)))
       	while rounded_up_time > 1: 
            if not self.bins.has_key(rounded_up_time) or self.bins[rounded_up_time] == 0.0:  
                self.bins[rounded_up_time] = 0.00  
            rounded_up_time = rounded_up_time / 2
    ...
    def __str__(self):
        s = str(self.number_of_jobs_added)
        for key in sorted(self.bins.keys()):
            s += " " + str(key) + ": " + str(self.bins[key]) + ", "
        return s 
            
        
class  ProbabilisticAlphaEasyScheduler(Scheduler):
    """ This algorithm implements a version of Feitelson and Nissimov, June 2007
    """
    
    def __init__(self, num_processors, threshold = 0.1):
        super(ProbabilisticAlphaEasyScheduler, self).__init__(num_processors)
    ...
    def _backfill_jobs(self, current_time):
        ...
        for job in tail:
            if self.user_jobs[job.user_id] != []: 
                ...
                predicted_run_time = max (1, int (alpha * job.user_estimated_run_time))
                j = Job(job.id, predicted_run_time, predicted_run_time, 1)
                d = Distribution()
                d.add_job(j)
                job_distribution = convex_merge(d, self.user_distribution[self.distribution_key(job)], 0.5)
            else:
                job_distribution = self.user_distribution[self.distribution_key(job)]
                 
            if self.can_be_probabilistically_backfilled(job, current_time, job_distribution):
    ...
    def can_be_probabilistically_backfilled(self, job, current_time, job_distribution):
        ...
        while t < rounded_up_user_estimation:
            job_probability_to_end_at_t = self.probability_to_end_at(t, job, job_distribution)
    ...
    def bottle_neck(self, time, second_job, first_job, current_time):
        ...
        for n in range(len(self.currently_running_jobs)):
            tmp_job = self.currently_running_jobs[n]
            # print "current job:", job
            Pn = self.probability_of_running_job_to_end_upto(time, current_time, tmp_job)
            # print "self.probability_of_running_job_to_end_upto", time, "is: ", Pn 
            for c in range (K + 1):
                # print "current c", c
                if c >= tmp_job.num_required_processors:  
                    M[n, c] = M[n-1, c] + (M[n-1, c - tmp_job.num_required_processors] - M[n-1, c]) * Pn
                    # print "case 1"
                elif c >= 1:
    ...
    def probability_to_end_at(self, time, job, job_distribution):         
